...
<head>
	...
	if (!empty($_POST)) {
		...
		// Qui comincia la transazione

		$stmtInsert = mysqli_prepare($connection, "INSERT INTO myorder(UserId, ProductId, Quantity) VALUES(?,?,?);");
		if (!$stmtInsert){
			error_log('Query error: ' . mysqli_error($connection));
			header("Location: ../home/executeError.php");
		}

		$stmtDelete = mysqli_prepare($connection, "DELETE FROM cart WHERE UserId=? AND ProductId=?;");
		if (!$stmtDelete){
			error_log('Query error: ' . mysqli_error($connection));
			header("Location: ../home/executeError.php");
		}

		$stmtUpdate = mysqli_prepare($connection, "UPDATE product SET Quantity = Quantity - ? WHERE Id = ?;"); // Ipotizzo che sia fatto un controllo lato DB che se il prodotto va a zero fallisce
		if (!$stmtUpdate){
			error_log('Query error: ' . mysqli_error($connection));
			header("Location: ../home/executeError.php");
		}
		...
		for ($i = 0; $i < count($key); $i++) {
			...
			// In tutti gli if dovrei mettere la rollback

			mysqli_stmt_bind_param($stmtInsert, 'iii', $_SESSION['Id'], $id, $quantity);
			if(!mysqli_stmt_execute($stmtInsert))
        		header("Location: ../home/executeError.php");
			
			mysqli_stmt_bind_param($stmtDelete, 'ii', $_SESSION['Id'], $id);
			if(!mysqli_stmt_execute($stmtDelete))
        		header("Location: ../home/executeError.php");

			mysqli_stmt_bind_param($stmtUpdate, 'ii', $quantity, $id);
			if(!mysqli_stmt_execute($stmtUpdate))
        		header("Location: ../home/executeError.php");

		}

		// Qui termina la transazione, in caso vada a buon fine
	}
